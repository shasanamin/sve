...
import wandb
from _datasets import IndexedDataset
...
class BaseTrainer:
    ...
    def train(self):
        ...
        # argument like resume_from_epoch would be overwritten by loaded model choice (generally 0)
        rfe_tmp = int(self.args.resume_from_epoch)
        if self.args.resume_from_epoch > 0:
            self._load_checkpoint(self.args.resume_from_epoch)
            self.args.resume_from_epoch = rfe_tmp

        # for epoch in range(self.args.resume_from_epoch, self.args.epochs):
        for epoch in range(self.args.resume_from_epoch-rfe_tmp, self.args.epochs-rfe_tmp):
            print(f"Training Epoch: {epoch+rfe_tmp}/{self.args.epochs}")
            ...
            print(f"Loss (Train): {self.train_loss.avg}, Acc (Train): {self.train_acc.avg}, Loss (Val): {self.val_loss}, Acc (Val): {self.val_acc}") 
                
            self.lr_scheduler.step()

            if (epoch+1) % self.args.save_freq == 0:
                # self._save_checkpoint(epoch)
                self._save_checkpoint(epoch+rfe_tmp)
    ...
    def _train_epoch(self, epoch):
        ...
        print(self.train_loader.dataset[0], self.train_loader.dataset[1])
        ...
        torch.save(
            {
                ...
                # "train_loss": self.train_loss,  # store AverageMeter() object for loading later; can take avg later on
    ...
    def _load_checkpoint(self, epoch):
        ...
        # self.train_loss = checkpoint["train_loss"]  # float instead of AverageMeter() will be loaded
