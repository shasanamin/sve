...
class CRESTRandomETrainer(SubsetTrainer):
    def __init__(
        self, 
        ...
        # keep to small value for initial testing
        self.switch_epoch=self.args.switch_epoch
        self.using_crest=True

    def _train_epoch(self, epoch):
        if epoch < self.switch_epoch:
            self._train_epoch_crest(epoch)
        else:
            self.using_crest=False
    ...
    def _train_epoch_crest(self, epoch: int):
        """
    ...
    def _select_subset(self, epoch: int, training_step: int):
        if self.using_crest:
            self._select_subset_crest(epoch, training_step)
        else:
            self._select_subset_random(epoch, training_step)
            
    
    def _select_subset_random(self, epoch: int, training_step: int):
        # select a subset of the data
        ...
        if warm_start and epoch<epochs_cold:
            ...
            with torch.no_grad():
                for _, (data, _, data_idx) in enumerate(train_val_loader):
                    ...
                    # pred_entropy[data_idx] = entropy(train_output[data_idx], axis=1)
            ...
            self.subset = np.random.choice(
                ...
                # p=pred_entropy / pred_entropy.sum()
            )

        self.subset_weights = np.ones(len(self.subset))
    
    def _select_subset_crest(self, epoch: int, training_step: int):
        """
        Select a subset of the data
        """
